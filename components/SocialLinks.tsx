    ...
    <div className={`flex items-center gap-3 ${className}`}>
      {NETWORKS.map(({ name, href, Icon }) => (
        <a
          ...
          aria-label={`Clay Highway no ${name}`}
          title={name}
          className="flex h-9 w-9 items-center justify-center rounded-full border border-border text-fog transition-colors hover:border-accent hover:bg-accent hover:text-ink"
        >
